...
#include "com.viraltaco.precondition.h" // vt_precondition
...
int_least64_t largest_series_product(char const* digits, const size_t span) {
  vt_precondition (digits != nullptr);
  const auto kLength = strlen(digits);
  ...
  auto max = (zeroes == 0) * tmp;
  
  for (int i = span; i != (int)kLength; ++i) {
    if (vt_not_digit(digits[i])) { return INVALID_SPAN; }
    
    const auto in  = vt_digit_at(i)
