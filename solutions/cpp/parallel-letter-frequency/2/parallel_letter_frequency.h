...
#define vt_parallel_letter_frequency "com.viraltaco.letter-frequency v" "0.2.0"
...
namespace parallel_letter_frequency {
 ...
 private:
  ...
  public:
    ...
    constexpr frequency_map(frequency_map const& map) noexcept
      : self(map.self)
      , zero(map.zero)
    {}
 ...
 public:
  constexpr frequency(record_type const& rec) 
    : self([&rec] {
      return std::reduce(PAR_UNSEQ       // Execution policy
        rec.cbegin()              // From
      , rec.cend()                // To
      , frequency_map{}// Into
